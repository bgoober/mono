...
export default function Bounties({session}: {session: Session | null}) {
  const {data: retrievedBounties, isLoading} = api.bounty.readAllBounties.useQuery()
  const [filter, setFilter] = useState<bountyFilter>({track:"ALL", status:"ALL"})
  ...
  return (
    <main className="m-auto flex w-full flex-1 flex-col px-5 sm:px-12">
      ...
      <section className="my-5 h-max flex flex-col md:flex-row flex-1 gap-3">
        <Sidebar filter={filter} setStatusFilter={setStatusFilter} setTrackFilter={setTrackFilter}/>
        ...
        <div className="flex-1">
          ...
          {isLoading && (
            <H4 className="pt-10 text-center text-zinc-600">
              Loading universal bounties <span className="animate-spin">⌛</span>
            </H4>
          )}
          {!isLoading && retrievedBounties?.filter(
            bounty => {
              const matchesSearch = bounty.name.includes(searchTerm)
              const matchesTrack = filter.track == "ALL" || bounty.track == filter.track
              const matchesStatus = filter.status == "ALL" || bounty.status == filter.status
